  ...
  switch (method) {
    case "GET":
      try {
        ...
        const filteredBids = bids.filter((rec: { houseId: number; }) => rec.houseId === houseId);
        ...
        console.log("/api/bids error:", err);
      }
    ...
    case "POST":
      try {
        ...
        recordFromBody.id = Math.max(...bids.map((bid: { id: number; }) => bid.id)) + 1;
